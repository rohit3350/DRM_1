...
async def download_video(url,cmd, name):
    download_cmd = f'{cmd} -R 25 --fragment-retries 25 --concurrent-fragments 4 --http-chunk-size 10M --external-downloader aria2c --downloader-args "aria2c:-x 16 -s 16 -k 1M -j 4 --max-connection-per-server=16 --min-split-size=1M"'
...
async def send_vid(bot: Client, m: Message, cc, filename, vidwatermark, thumb, name, prog, channel_id):
    proc = await asyncio.create_subprocess_shell(f'ffmpeg -i "{filename}" -ss 00:00:10 -vframes 1 "{filename}.jpg"')
    await proc.wait()
    ...
    try:
        ...
        if vidwatermark == "/d":
            ...
            wm_proc = await asyncio.create_subprocess_shell(
